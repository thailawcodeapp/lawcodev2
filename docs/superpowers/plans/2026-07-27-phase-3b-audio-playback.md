...
- Bumping the app version means bumping **three** numbers, to the same value, in the same commit:
  - `APP_VERSION_CODE` in `src/config.js` — shown in Settings and used by the update check
  - `versionCode` in `android/app/build.gradle` — the Play Store's build number
  - `CURRENT_PROJECT_VERSION` in `ios/App/App.xcodeproj/project.pbxproj` (**two occurrences**, Debug and Release) — the iOS build number

  The third was missed once and the symptom was silent: nothing in the build pipeline connects the Xcode project to the other two, so a release with the first two bumped still carried the previous `CFBundleVersion`. App Store Connect refuses a build number it already has, the upload step is `continue-on-error`, and the job finished green with nothing in TestFlight. They are aligned to a single number now so there is one thing to remember rather than three. `APP_VERSION_NAME` must equal `versionName` in that same gradle file — the settings screen renders both, so a mismatch is visible and misleads bug reports. Read the current numbers before changing them; they moved to 57 after this plan was written.

**UI conventions introduced by the Android work that merged into `cloud-sync` on 2026-08-03.** They are not optional, and each one exists because its absence was a shipped bug:

- **No `window.confirm()` or `alert()`.** In a WebView they render with the origin as a title — users saw "localhost says:". Use `src/components/ConfirmDialog.jsx`.
- **No hard-coded hex for rules, dividers or row shading.** Use `var(--rule-hair)`, `var(--rule-strong)`, `var(--row-alt)`, which flip for dark mode. Hard-coded values made every divider vanish in dark mode.
- **Every new button or tappable row needs press feedback:** `.tap-row` for a full-width row on the paper surface, `.tap-btn` for a button or anything on a dark surface. Tailwind is configured `hoverOnlyWhenSupported`, so `hover:` does nothing on touch.
- **Anything with a tap target under 44px gets `.hit-44`**, which grows the target through `::after` without changing what is drawn. Do not add it to buttons that sit adjacent and do different things — the invisible boxes would overlap; space them apart instead.
- **Never `key={index}`** on a list whose contents swap in place. It made a press highlight ride a recycled row into the next level.
- **No `requestAnimationFrame` for mount animation** — it does not fire while the page is not compositing. Use CSS `@keyframes` and make the resting style already correct.
- Reuse `src/components/BottomSheet.jsx`, `ConfirmDialog.jsx`, `ProGateModal.jsx`, and `showToast()` from `src/lib/toast.js`. Do not build new ones.
...
- Modify: `src/components/TtsPlayer.jsx:138-140` (the sub-label line, now at :140)
...
- Modify: `src/screens/SettingsScreen.jsx` — inside the existing `<Group title="เสียงอ่าน">`, which now opens at :297 and closes at :343
...
Do not create a second `<Group title="เสียงอ่าน">`. One already exists (opens at :297) and this belongs inside it, just before its closing `</Group>` at :343.
...
The Android UI work landed on `cloud-sync` while this plan was being written and took the build number with it. Read the current values rather than trusting any number written here or remembered from earlier:

```bash
grep -n "versionCode\|versionName" android/app/build.gradle && grep -n "APP_VERSION_CODE\|APP_VERSION_NAME" src/config.js
```

At the time of writing all four read: `versionCode 57`, `versionName "1.0.4"`, `APP_VERSION_CODE = 57`, `APP_VERSION_NAME = '1.0.4'`. So this release is **58**, set in both `src/config.js` and `android/app/build.gradle`.

`APP_VERSION_NAME` is new and must stay equal to `versionName` in the gradle file. The settings screen now renders `v{APP_VERSION_NAME} · build {APP_VERSION_CODE}` instead of a hard-coded "v1.0", so a mismatch is visible to every user and misleads every bug report. Leave `versionName` at `1.0.4` unless the release actually changes it — and if it does, change both.
