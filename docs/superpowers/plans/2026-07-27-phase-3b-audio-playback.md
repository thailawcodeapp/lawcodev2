...
- Bumping the app version means bumping **both** `APP_VERSION_CODE` in `src/config.js` **and** `versionCode` in `android/app/build.gradle`. `APP_VERSION_NAME` must equal `versionName` in that same gradle file — the settings screen renders both, so a mismatch is visible and misleads bug reports. Read the current numbers before changing them; they moved to 57 after this plan was written.
