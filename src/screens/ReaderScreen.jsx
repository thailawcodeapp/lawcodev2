...
function renderHighlightedText(text, highlights) {
  ...
  for (const hl of sorted) {
    if (hl.startOffset > cursor) parts.push({ text: text.slice(cursor, hl.startOffset), hl: null });
    parts.push({ text: text.slice(hl.startOffset, hl.endOffset), hl });
    cursor = hl.endOffset;
  }
  ...
  return parts.map((part, i) => {
    ...
    return (
      <mark
        key={i}
        style={{ background: style.bg, borderRadius: 3, padding: '1px 1px' }}
...
export default function ReaderScreen() {
  ...
  // Is the global player currently reading THIS section?
  ...
  useEffect(() => {
    if (section) {
      ...
      setNotes(getNotesForSection(section.id));
      setHlPopup(null);
    }
  ...
  // Auto-scroll the active paragraph while reading aloud
  useEffect(() => {
    if (activePara >= 0 && paraRefs.current[activePara]) {
  ...
  // Speaker button → start reading immediately (#9), or stop if already reading this
  const handleTts = () => {
    if (ttsThis) { tts.stop(); return; }
    const item = buildSectionItem({
      sectionId: section.id, bookId: book.id, number: section.number,
      title: section.title, paragraphs: bodyParagraphs,
    });
    tts.playSections([item], 0);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-paper dark:bg-dark-bg text-ink dark:text-paper font-serif overflow-hidden">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto pb-20">
        <div className="px-5">
          ...
          <div className="pt-5 pb-3.5 border-b-2 border-rule dark:border-paper">
            <div className="font-ui text-[9px] tracking-[2px] uppercase font-bold text-accent">มาตรา</div>
            <div
              className="font-display font-extralight leading-none text-ink dark:text-paper"
              style={{ fontSize: 'clamp(80px, 22vw, 150px)', letterSpacing: -6, lineHeight: 0.85, fontVariantNumeric: 'lining-nums', marginTop: 4 }}
            >
              {section.number}
            </div>
            {cleanTitle && (
              <div className="font-display font-medium italic leading-snug mt-3" style={{ fontSize: 20, letterSpacing: -0.3 }}>
                {cleanTitle}
              </div>
            )}
            <div className="flex gap-3 mt-3 font-ui text-[10px] text-ink-soft dark:text-rule-soft tracking-wide">
              <span className="font-semibold text-ink dark:text-paper">{book.shortName}</span>
              <span>มาตรา {section.number}</span>
            </div>
          </div>

          {/* Highlight count + clear */}
          {highlights.length > 0 && (
            <div className="flex items-center gap-2 py-2 border-b border-rule-soft/50 dark:border-ink-soft/50">
              <div className="font-ui text-[10px] text-ink-soft dark:text-rule-soft">ไฮไลท์ {highlights.length} จุด</div>
              <button
                onClick={() => { highlights.forEach(h => deleteHighlight(section.id, h.id)); setHighlights([]); }}
          ...
          {/* Notes shown below the section text (#13) */}
          {notes.length > 0 && (
            <div className="border-t border-rule dark:border-ink-soft pt-3.5 pb-2">
              <div className="flex items-center justify-between mb-2">
                <div className="font-ui text-[9px] tracking-[2px] uppercase font-bold text-accent">บันทึกของฉัน</div>
                <button onClick={() => setShowNotes(true)} className="font-ui text-[10px] text-ink-soft dark:text-rule-soft underline">
                  แก้ไข
                </button>
              </div>
              {notes.map(n => (
                ...
                return (
                  ...
                  >
                    <div className="flex items-baseline gap-3">
                      <span className="font-display font-normal italic text-ink dark:text-paper" style={{ fontSize: 22, fontVariantNumeric: 'lining-nums' }}>
                        §{ref}
                      </span>
                      <span className="font-serif text-[13px] text-ink-soft dark:text-rule-soft">{refTitle}</span>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="m9 6 6 6-6 6" /></svg>
                  </button>
                );
              })}
            </div>
      ...
      <HighlightPopup
        ...
        onDismiss={() => { setHlPopup(null); window.getSelection()?.removeAllRanges(); }}
      />
    </div>
  );
}
